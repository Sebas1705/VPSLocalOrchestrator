# Docs Index

Version: v1.0.3

- **Project overview**: `../README.md`
- **Reference** (auth, endpoints, testing, deploy): `REFERENCE.md`
- **Setup & architecture**: `SETUP_ARCHITECTURE.md`
- **Security & examples**: `SECURITY_EXAMPLES.md`
- **Operations** (troubleshooting): `OPERATIONS.md`
- **Development** (git, contribution, AI protection): `DEVELOPMENT.md`
- **Roadmap & releases**: `ROADMAP_RELEASES.md`
- **Quick sheet**: `QUICK_REFERENCE.md`
- **Testing details**: `core/TESTING.md`
- **Coverage report**: `../TEST_COVERAGE_EXPANSION_REPORT.md`
- **n8n integration**: `N8N_INTEGRATION.md`

